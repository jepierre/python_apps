...
class Main(QMainWindow):
    ...
    def save_as_file(self):
        self.save_file_name = QFileDialog.getSaveFileName(self, "Save Result", self.save_file_name, "Excel Files (.xlsx);;All (*.*)")[0]
        logger.debug(f'save_file_name: {self.save_file_name}')
        if self.save_file_name and self.result is not None:
            self.result.to_excel(self.save_file_name)
    ...
    def calculate_total(self):
        ...
        for idx, month in enumerate(self.months):
            ...
            mask = (df.index >= f'{self.year}-{idx}-1') & (df.index <= f'{self.year}-{idx}-{last_day}')
        ...
        self.result = self.result.append(self.result.sum(numeric_only=True).rename('Total'))
    ...
    def update_table(self):
        ...
        for inx, column in enumerate(self.result.columns):
            for iny, index in enumerate(self.result.index):
                ...
                self.main_table.setItem(iny, inx, QTableWidgetItem(f'$ {self.result[column][index]:,.2f}'))
